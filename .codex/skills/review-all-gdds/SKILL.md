...
the same GDD inputs but produce separate reports. Spawn both as parallel child-agent
...
Use `structured user input (or one concise question)` for write permission:
- Prompt: "May I write this review to `design/gdd/gdd-cross-review-[date].md`?"
- Options: `[A] Yes — write the report` / `[B] No — skip`

If any GDDs are flagged for revision, use a second `structured user input (or one concise question)`:
...
After all file writes are complete, use `structured user input (or one concise question)` for a closing widget.
...
3. **Offer options** via structured user input (or one concise question) with three choices:
