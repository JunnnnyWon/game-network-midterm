...
After completing the engine audit above, spawn the **primary engine specialist** via Codex native child agents for a domain-expert second opinion:
...
Use `structured user input (or one concise question)` for the write approval:
...
Then close with `structured user input (or one concise question)`:
