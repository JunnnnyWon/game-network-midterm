...
- If it exists: read it, find the first context (system/level/character) with any asset at status "Needed" but no spec file written yet, and use `structured user input (or one concise question)`:
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  ...
  Use `structured user input (or one concise question)`: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
Present the full identified list to the user. Use `structured user input (or one concise question)`:
...
Spawn specialist agents based on review mode. **Issue all Codex native child agents calls simultaneously — do not wait for one before starting the next.**

### Full mode — spawn in parallel:

**`art-director`** via Codex native child agents:
...
**`technical-artist`** via Codex native child agents:
...
After presenting all specs, use `structured user input (or one concise question)`:
