...
**Decision Points:** At each phase transition, use `structured user input (or one concise question)` to present
...
Spawn `qa-lead` via Codex native child agents to review all in-scope stories and produce a QA strategy.
...
Present the qa-lead's full strategy to the user, then use `structured user input (or one concise question)`:
...
Spawn `qa-tester` via Codex native child agents for each story (run in parallel where possible), providing:
- The story file path
...
Use `structured user input (or one concise question)` per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use `structured user input (or one concise question)` for each:
...
After each FAIL result: use `structured user input (or one concise question)` to collect the failure description, then spawn `qa-tester` via Codex native child agents to write a formal bug report in `production/qa/bugs/`.
...
Spawn `qa-lead` via Codex native child agents to produce the sign-off report using all results from Phases 4–6.
