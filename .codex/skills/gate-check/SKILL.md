...
Before generating the final verdict, spawn all four directors as **parallel subagents** via Codex native child agents using the parallel gate protocol from `.claude/docs/director-gates.md`. Issue all four child-agent spawns simultaneously — do not wait for one before starting the next.
...
After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt using `structured user input (or one concise question)`.
...
- **No accessibility requirements doc?** → Use `structured user input (or one concise question)` to offer to create it now:
  ...
  - If "Create it now": use a second `structured user input (or one concise question)` to ask for the tier:
