...
user-invocable: true
allowed-tools: Read, Glob, Grep, Write, Edit, spawn_agent, request_user_input, TodoWrite
---
...
> Usage: `/team-narrative [narrative content description]` — describe the story content, scene, or narrative area to work on (e.g., `boss encounter cutscene`, `faction intro dialogue`, `tutorial narrative`). Do not use `structured user input (or one concise question)` here; output the guidance directly.
...
Delegate in parallel — issue all three child-agent spawns simultaneously before waiting for any result:
...
sub-agents spawned via Codex native child agents. Each sub-agent enforces the "May I write to [path]?"
