...
user-invocable: true
allowed-tools: Read, Glob, Grep, Write, Edit, Bash, spawn_agent, request_user_input, TodoWrite
---
...
**Decision Points:** At each step transition, use `structured user input (or one concise question)` to present
...
Use Codex native child agents (or OMX orchestration lanes) to spawn each team member role:
...
Spawn all three agents simultaneously — issue all three child-agent spawns before waiting for any result.
...
**Gate**: Use `structured user input (or one concise question)` to present all three Step 1 outputs (narrative brief, lore foundation, visual direction targets) and confirm before proceeding to Step 2.
...
Use `structured user input (or one concise question)` with options:
...
**Gate**: Use `structured user input (or one concise question)` to present Step 2 layout (including any unresolved adjacent area dependencies) and confirm before proceeding to Step 3.
...
**Gate**: Use `structured user input (or one concise question)` to present Step 3 outputs and confirm before proceeding to Step 4.
...
**Gate**: Use `structured user input (or one concise question)` to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:
...
to sub-agents spawned via Codex native child agents. Each sub-agent enforces the "May I write to [path]?"
