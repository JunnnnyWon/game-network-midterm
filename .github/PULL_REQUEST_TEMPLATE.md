## Summary

Brief description of what this PR does.

## Type of Change

- [ ] New agent
- [ ] New skill
- [ ] New hook or rule
- [ ] Bug fix
- [ ] Documentation improvement
- [ ] Other:

## Changes

-
-
-

## Checklist

- [ ] I've tested this in the intended runtime (Claude or Codex/OMX)
- [ ] New agents include the Collaboration Protocol section
- [ ] New skills use the correct runtime surface (`.claude/skills/<name>/SKILL.md` for Claude source, `.codex/skills/<name>/SKILL.md` for Codex)
- [ ] Reference docs are updated (agent-roster, skills-reference, hooks-reference, rules-reference)
- [ ] Hooks use `grep -E` (POSIX) and fail gracefully without jq/python
- [ ] No hardcoded paths or platform-specific assumptions
